...
export function AriaSwitch({ children, className, ...props }: AriaSwitchProps) {
    return (
        ...
        >
            <div className="w-11 h-6 bg-[#d5dde2] rounded-full p-1 transition-colors duration-200 group-data-[selected=true]:bg-[#3f52ff]">
                <div className="bg-white w-4 h-4 rounded-full shadow-sm transform transition-transform duration-200 group-data-[selected=true]:translate-x-5" />
            </div>
            {children}
        </Switch>
    );
